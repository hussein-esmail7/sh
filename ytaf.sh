...
--no-warnings --no-part -f best --audio-format mp3 \
-o "%(channel)s/%(title)s.%(ext)s" "$string"
# Run yt-dlp with 100 retries
# get best quality of audio only in MP3 format
# download all in a playlist if it is one
# do not use part files
# Put into folders (ideal if downloading from multiple channels in one command)
